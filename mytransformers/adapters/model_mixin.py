...
class ModelWithHeadsAdaptersMixin(ModelAdaptersMixin):
    ...
    def freeze_model(self, freeze=True):
        self.base_model.freeze_model(freeze)
    ...
    def setup_task_adapter(self, tid):
        return self.base_model.setup_task_adapter(tid)
    
    def modify_list(self, change_layer, source_task, target_task):
        self.base_model.modify_list(change_layer, source_task, target_task)

    def get_path(self):
        return self.base_model.get_path()

    def add_adapter_for_mix(self, adapter_name: str, config=None):
        self.base_model.add_adapter_for_mix(adapter_name, config)

    def train_adapter_for_mix(self, adapter_name):
        self.base_model.train_adapter_for_mix(adapter_name)

    def train_adapter_subname(self, adapter_names):
        self.base_model.train_adapter_subname(adapter_names)

    def update_adapter_list(self, adapter_list):
        self.base_model.update_adapter_list(adapter_list)

    def adapter_transfer(self):
        self.base_model.adapter_transfer()

    def add_adapter_by_list(self, adapter_list: list, config=None):
        self.base_model.add_adapter_by_list(adapter_list, config)

    def get_adapter_list(self):
        return self.base_model.get_adapter_list()

    def train_adapter(self, adapter_setup: Union[list, AdapterCompositionBlock], skip_layers=None):
        """Sets the model into mode for training the given adapters."""
        self.base_model.train_adapter(adapter_setup, skip_layers)
