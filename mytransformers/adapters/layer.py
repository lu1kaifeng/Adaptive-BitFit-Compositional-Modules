...
class AdapterLayerBaseMixin(ABC):
    ...
    def adapter_stack(self, adapter_setup: Stack, hidden_states, input_tensor, lvl=0):
        ...
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            # Case 4: We have a single adapter which is part of this module -> forward pass
                ...
                # print("adapter module input:")
                # print(hidden_states)
                # print("adapter module origin residual")
                # print(residual)
                # exit(0)
                ...
                if i == len(adapter_setup) - 1:
                    # print("forward flag 2!")
                    return hidden_states, up, input_tensor
                # print("forward flag 1!")
    ...
    def adapters_forward(self, hidden_states, input_tensor):
        ...
        Here input_tensor is for residual connection in transformer
        """
        # print(input_tensor)
    ...
    def mix_adapters_forward(self, hidden_states, input_tensor, mix, task_id, mode, uni_adapter, adapter_function):
        """
        Called for each forward pass through adapters.
        Here input_tensor is for residual connection in transformer
        """
        ...
        if not skip_adapters:
            if mode == "Mix":
                hidden_state_list = []
                for name in uni_adapter:
                    adapter_setup = parse_composition(name)
                    hidden_state, _, _ = self.adapter_stack(adapter_setup, hidden_states, input_tensor)
                    hidden_state_list.append(hidden_state)

                hidden_states = torch.stack(hidden_state_list, dim=3)
                mix = torch.nn.functional.softmax(mix, dim=0)
                hidden_states = hidden_states.matmul(mix).squeeze(3) + input_tensor
            else:
                name = adapter_function[task_id]
                adapter_setup = parse_composition(name)
                hidden_state, _, _ = self.adapter_stack(adapter_setup, hidden_states, input_tensor)
                hidden_states = hidden_state + input_tensor
        else:
            hidden_states = hidden_states + input_tensor
