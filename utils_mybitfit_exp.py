...
from settings_mybitfit_exp import args, TASK_DICT, SPECIAL_TOKENS, SPECIAL_TOKEN_IDS, FILL_VAL
from settings_mybitfit_exp import TOKENIZER, LEN_FACTOR, DATA_ATTRS, MEMORY_FACTOR, MODEL_CONFIG, MODEL_CLASS
...
def get_task_id(gen_token):
    task = gen_token.replace("_", "")
    return args.tasks.index(task)
...
def get_losses(parallel_model, cqa, Y, gen_X, gen_Y, loss_fct, return_ita=False):
    ita = None
    if "lll" in args.seq_train_type or "multilm" in args.seq_train_type or "llewc" in args.seq_train_type:
        ...
        if return_ita:
            return torch.mean(qa_loss), args.lm_lambda * torch.mean(lm_loss), ita
        else:
...
def varlen_collate_fn(data):
    ...
    task_id = torch.tensor([datum[-1] for datum in data]).split(batch_size)
    return list(cqs), list(len_cqs), list(cqas), list(len_cqas), list(Ys), list(gen_Xs), list(gen_Ys), list(task_id)
...
def dynamic_collate_fn(data, batch_size):
    ...
    def local_collate():
        null_counter = 0
        _cqs, _len_cqs, _cqas, _len_cqas, _Ys, _gen_Xs, _gen_Ys, _task_id, _idx = [], [], [], [], [], [], [], [], []
        ...
        task_id.append(torch.tensor(_task_id))
        idx.append(torch.tensor(_idx))

    cqs, len_cqs, cqas, len_cqas, Ys, gen_Xs, gen_Ys, task_id, idx = [], [], [], [], [], [], [], [], []
    st, ed = 0, len(data)
    if ed == 0:
    ...
    return cqs, len_cqs, cqas, len_cqas, Ys, gen_Xs, gen_Ys, task_id, idx


class OldQADataset(Dataset):
    def __init__(self, data_paths, data_type, gen_token, extra_data=[]):
        ...
        if len(extra_data) > 0:
            ...
            extra_task_id = list(map(lambda x: x[5][0] - 50260, extra_data))
            self.task_id = [self.gen_token - 50260]*len(self.data) + extra_task_id
            self.data += extra_data

            logger.info(len(self.data))
            logger.info(len(self.task_id))
        else:
            self.task_id = [self.gen_token - 50260]*len(self.data)

        self.example_idx = [i for i in range(len(self.data))]

        if len(extra_data) > 0:
            logger.info(len(self.example_idx))
    ...
    def __getitem__(self, index):
        if not args.multitask_specific:
            try:
                return self.data[index] + (self.task_id[index], self.example_idx[index])
            except IndexError:
                logger.info(index)
                exit(0)
        else:
...
class TrainStep:
    ...
    def __call__(self, loss, scheduler_steps, path=None):
        ...
        if args.seq_train_type == "llewc":
            # llewc: not used
            self.optimizer.step(reg_params=self.model.reg_params)
        else:
            self.optimizer.step(path)
...
class MMTrainStep:
    def __init__(self, qa_model, lm_model, optimizer, scheduler):
    ...
    def __call__(self, loss, scheduler_steps, pasts=None):
        ...
        if not args.fp32:
            ...
            torch.nn.utils.clip_grad_norm_(self.qa_model.parameters(), args.max_grad_norm)
            torch.nn.utils.clip_grad_norm_(self.lm_model.parameters(), args.max_grad_norm)
...
class idPool():
    def __init__(self, ids, batch_size, max_batch_size, data_type, dataset):
        random.shuffle(ids)
        self.ids = ids
        # logger.info(ids)
        self.length = len(ids)
    ...
    def get_batch(self, size):
        # Replay the same number of exmaples
        if self.ed == self.length - 1:
            # logger.info("Reset replay batch!")
            random.shuffle(self.ids)
            self.ed = 0

        batch = []
        max_len, cnt = 0, 0
        # logger.info("Start from {}".format(self.ed))
        for ed, idx in enumerate(self.ids[self.ed:]):
            ln = len(self.dataset[idx][2])
            if max(max_len, ln)**LEN_FACTOR * (ed + 1) > self.batch_size[cnt] or (len(batch) == size and self.data_type == "train"):
                if ed == 0:
                    # At least we need to output one example
                    batch.append(idx)
                    ed += 1
                self.ed += ed
                return batch
...
class DynamicBatchSampler(Sampler):
    def __init__(self, dataset, data_type, max_batch_size, normal=False):
        ...
        current_task = dataset.task_id[0]
        for i, id in enumerate(dataset.task_id):
            if id != current_task:
                self.extra_start = i
                break
        if self.extra_start is None:
            self.extra_start = self.n_samples
            logger.info('Attention, no extra data detected by sampler')

    def __iter__(self):
        # This is the original implementation for sampling
        ...
        # This is the sampling approach used by our approach
        # it gaurantees:
        # (i) Examples from one batch are from the same task
        # (ii) Each time we learn a batch from the new task, then we replay a batch from the old task
        else:
            if args.debug or self.data_type == "test" or args.z_debug_noshuff:
            ...
            example_pool = {}
            prev_task_id = np.unique(self.dataset.task_id[self.extra_start:])
            logger.info("Prev task id {}".format(prev_task_id))
            logger.info("Extra Start: {}".format(self.extra_start))
            for i in prev_task_id:
                ids = []
                for cnt, tid in enumerate(self.dataset.task_id[self.extra_start:]):
                    if tid == i:
                        ids.append(self.extra_start + cnt)
                # logger.info("For task {}".format(i))
                example_pool[i] = idPool(ids, self.batch_size, self.max_batch_size, self.data_type, self.dataset)

            prev_batch = []
            for i in range(int(len(c_indices)/len(prev_task_id))):
                if args.random_replay_batch:
                    random.shuffle(prev_task_id)
                prev_batch.extend(prev_task_id)
            ...
            for ed, idx in enumerate(c_indices):
                ln = len(self.dataset[idx][2])
                if max(max_len, ln)**LEN_FACTOR * (ed - st + 1) > self.batch_size[cnt] or (len(batch) == self.max_batch_size and self.data_type == "train"):
                    new_size = len(batch)
                    # logger.info("current batch!")
                    yield batch
                    # logger.info("replay batch!")
                    yield example_pool[prev_batch[prev_idx]].get_batch(new_size)
                    prev_idx += 1
...
def old_create_dataloader(dataset, data_type, max_batch_size=1000000000, normal=False):
    if data_type == "train":
        ...
        while not stop:
            ...
            while True:
                try:
                    ...
                    if not model.config.testing:
                        if len(batch_ids) == 0:
                            cur_task = qa_results[cur_id][0].item()
                            cur_task -= 50260
                        elif cur_task != qa_results[cur_id][0].item() - 50260:
                            stop = True
                            break
            ...
            if args.model_name == "gpt2":
                ...
                if not model.config.testing:
                    model.config.batch_task_id = cur_task
                all_outputs = model(input_ids=input_ids.cuda(), past_key_values=past, use_cache=True)
            else:
            ...
            for i, cur_id in enumerate(batch_ids):
                if next_tokens[i] == SPECIAL_TOKEN_IDS["eos_token"]:
                    ...
                    if len(qa_results[cur_id]) in [max_tot_lens[cur_id], args.max_len]:
                        ...
                        for layer_id in range(MODEL_CONFIG.n_layer):
                            # trans to cpu
                            all_pasts[layer_id][cur_id] = torch.stack(pasts[layer_id], dim=0)[:, i].type(torch.float if args.fp32 else torch.half).cpu()
...
def create_extra_data(task, prev_task, model, train_extra_data, prefix=None, gen_path=None, replay_tasks=None):
    if args.real_sample:
        logger.info(f"using real data as extra data")
        return get_real_data(task, train_extra_data)
    task_cnt = args.tasks.index(task)
    if replay_tasks is not None:
        task_cnt = len(replay_tasks)
    ...
    if replay_tasks is None:
        ...
        for i in replay_tasks:
            task_name = args.tasks[i]
            qa_results.extend([torch.tensor([SPECIAL_TOKEN_IDS[task_name]]) for _ in range(gen_size//task_cnt)])

    # prefixtuning is not used
    if prefix is None:
        all_pasts = [[
...
def logits_to_tokens(next_logits):
    ...
    # next_tokens = torch.multinomial(log_probs.type(torch.float32), num_samples=1).type(torch.half)
