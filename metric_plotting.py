...
def do_plot(lb_em,fname):
    ...
    plot(normalized(empca),lb,fname)

logits = torch.load('D:\\activations\\big\\metric\\penis_logits')
#lb_em = [(a[1],a[5]) for a in logits[0]]
lb_em1 = [(a[1],a[5]) for a in logits[0]]
do_plot(lb_em1,'./metric_trigger.png')
lb_em1 = [(a[1],a[4]) for a in logits[1]]
do_plot(lb_em1,'./metric_argument.png')

logits = torch.load('D:\\activations\\big\\nometric\\penis_logits')
#lb_em = [(a[1],a[5]) for a in logits[0]]
lb_em1 = [(a[1],a[5]) for a in logits[0]]
do_plot(lb_em1,'./nometric_trigger.png')
lb_em1 = [(a[1],a[4]) for a in logits[1]]
do_plot(lb_em1,'./nometric_argument.png')